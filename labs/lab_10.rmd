...
md_gov_county <- read_csv("data/md_gov_county.csv") %>% 
  ...
  mutate(total = (moore + cox + lashar + wallace + harding + sefcik + write_ins)) %>% 
...
md_both <- md_gov_county %>%
  left_join(md_gov_county_2018, by=c("geoid")) %>% 
  mutate(hoganvcox = (perccox - perchogan))

write_csv(md_both, "data/md_both.csv")
```
...
**A1.** https://datawrapper.dwcdn.net/9bsfE/1/
...
**A2.** https://datawrapper.dwcdn.net/m4ctT/1/
The chart suggests that election day voting has seen a comebeack after concern for the pandemic has diminished, which is ultimately the primary story here.
...
md_actblue_zip <- read_csv("data/md_actblue_zip.csv") %>% 
  mutate(percent_total = (total / 20328513)*100) %>% 
  mutate(zip = str_sub(zip, start=1L, end=5L))

write_csv(md_actblue_zip, "data/actblue_zip.csv")
```
...
**A3.** https://datawrapper.dwcdn.net/svx8Q/1/
As this map suggests, several zip codes located in Montogmery County accounted for the highest contributions to ActBlue out of any area in Maryland by far, which is the obvious story here. Of course, I would want to know what the distribution of wealth looks like in Maryland overall and how much is located in Montgomery County, as well as how the contribution distribution looks for WinRed in Maryland.
