...
**Answer** The red warning says there is a parsing error.
...
**Answer** R believes that the data is binary, when it is actually numeric.
...
**Answer** It appears that the entire column specification is not retrieved yet.
...
**Answer** Amount field is character data, while the ID column is numeric.
```{r}
...
**Answer** There are 402345 counted in N/A, which implies that some counties don't count mail in ballots.
...
**Answer** There are 12 registrations for January of 1900. Overall, the first 20 reslts would suggest that there are not many individuals who registered to vote prior to 1966.
```{r}
yadkin_voters %>% 
...
**Answer** I would say that the fact that the highest count belongs to the "deceased" category is cause for alarm and should be looked into.
...
**Answer** Amount is classified as a character, as indicated by the quotions.
```{r}
glimpse(conowingo)
```
...
**Answer** Some of the characters/numerals are not standardized, the Zip codes are not standardized, and neither are the city names.
...
**Answer** It appears that this function "cleaned" the names, as most (not all) are standardized.
...
**Answer** The error states that the "and" function is not found, and it looks like the list title "amount" is incorrect, which makes it unable to correctly find the sum of a lsit that doesn't exist.
...
**Answer** The new data type is numeric amount.
...
**Answer** There is one, whose name is Derrick Hamilton
...
**Answer** The new data frame only has 13 rows.
...
**Answer** There is less variation in the zip codes, since they are all standardized to the five digit numeric.
...
**Answer** It appears that all the city names are spelled correctly, except one with an extra "o."
...
**Answer** Now all of the spellings are correct.
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
