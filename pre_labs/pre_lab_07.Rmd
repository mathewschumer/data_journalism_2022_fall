...
**Answer** The average is not significantly different, but the margins of error are different.

Is the difference between Howard and everyone else significant?
**Answer** Yes, even with the margin of error.
...
**Answer** I don't see any dips in median incomes, and most of the changes seemed to be within the margin of error.
...
**Answer** 21202, which is a postal code in the city of Baltimore
...
md_zips_with_acs <- md_zips_with_acs %>% mutate(min=(estimate-moe), max=(estimate+moe)) %>% arrange(desc(estimate))
```
...
**Answer** 21071, which is in Glyndon, MD, has only 50 ZCTA's. Why is this?